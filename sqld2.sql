...
5. Select DepartmentID From Departments where DepartmentName='Accounting';
   UPDATE Employees SET Email='not available' WHERE DepartmentID=110;

6. UPDATE Employees SET Salary=8000 WHERE ID=105AND Salary<5000;
...
3. SELECT FirstName FROM Employee AS EmployeeName ;
...
6. SELECT FirstName FROM Employee AS Employee Name WHERE NOT FirstName='Neena';

7. SELECT * FROM Employees WHERE Salary BETWEEN 5000 AND 8000;

8. SELECT FirstName,LastName,Salary,Salary*.12 AS PF FROM Employees;
...
13. SELECT TOP 2 Salary FROM Employee;
